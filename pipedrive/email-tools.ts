...
export class EmailTools {
    ...
    async sendEmail(params: z.infer<typeof SendEmailSchema>) {
        try {
            ...
            let response;
            try {
                response = await (this.client as any).request('/mailbox', {
                    method: 'POST',
                    body: emailData
                });
            } catch (apiError: any) {
                ...
                response = await this.client.createActivity(activityData);
    ...
    async getEmailTemplates(params: z.infer<typeof GetEmailTemplatesSchema>) {
        try {
            const validated = GetEmailTemplatesSchema.parse(params);
            const response = await (this.client as any).request('/emailTemplates', {
                params: validated
            });
    ...
    async createEmailTemplate(params: z.infer<typeof CreateEmailTemplateSchema>) {
        try {
    ...
    async getEmailPerformance(params: z.infer<typeof GetEmailPerformanceSchema>) {
        try {
            ...
            if (validated.pipeline_id) {
                ...
                const dealIds = dealsResponse.data?.map((deal: any) => deal.id) || [];
                const emailStats = {
                    total_emails: 0,
                    deals_with_emails: 0,
                    pipeline_id: validated.pipeline_id,
                ...
                for (const dealId of dealIds.slice(0, 50)) { // Limit for performance
                    try {
                        const activitiesResponse = await this.client.getActivities({
                            deal_id: dealId
                        });
                ...
                const response = await this.client.getActivities(activityParams);
    ...
    async trackEmail(params: z.infer<typeof TrackEmailSchema>) {
        try {
            ...
            const response = await this.client.createActivity(activityData);
    ...
    async getEmailConversations(params: any) {
        try {
            ...
            if (validated.deal_id) {
                response = await this.client.getActivities({
                    deal_id: validated.deal_id,
                    limit: validated.limit
                });
