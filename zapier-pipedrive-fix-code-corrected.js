// Zapier Code Step - Fixed Pipedrive Integration (CORRECTED)
// Copy this code into a Zapier "Code by Zapier" step

// Input from webhook or previous step - USE inputData instead of input
...
async function processCompany() {
  ...
  // Only search Pipedrive if we have an API token
  if (pipedrive_api_token) {
    promises.push(searchPipedriveOrganization(cleanCompanyName, pipedrive_api_token));
  } else {
    promises.push(Promise.resolve({ success: false, message: 'No Pipedrive API token provided' }));
  }

  // Always get company insights
  promises.push(getCompanyInsights(cleanCompanyName));

  const [pipedriveResult, insightsResult] = await Promise.all(promises);
